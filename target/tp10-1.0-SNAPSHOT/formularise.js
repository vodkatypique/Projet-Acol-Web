...
function formularise(lien, event, id, action) {
    ...
    var ligne = lien.parentElement.parentElement;
    var cases = ligne.cells;
    var auteur = cases[0].textContent;
    var titre = cases[1].textContent;
    var ancienne_ligne = ligne.innerHTML;
    if(action === "supprimer") {
    ...
    cases[2].innerHTML = "<input type='hidden' name='action' value='" + action + "' />" + 
...
function formularise_correction(lien, event, id, action) {
    "use strict";
